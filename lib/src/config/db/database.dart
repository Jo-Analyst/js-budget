  ...
  static Future<sql.Database> openDatabase() async {
    ...
    return sql.openDatabase(
      ...
      onCreate: (db, version) {
        ...
        db.execute(
            'CREATE TABLE IF NOT EXISTS items_budget (id INTEGER PRIMARY KEY, sub_value REAL, unitary_value REAL, term INTEGER, time_incentive TEXT, percentage_profit_margin REAL, profit_margin_value REAL, product_id INTEGER, service_id INTEGER, budget_id INTEGER, FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE SET NULL, FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE SET NULL, FOREIGN KEY (budget_id) REFERENCES budgets(id) ON DELETE SET NULL)');
